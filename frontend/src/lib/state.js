...
export function importResidentialAndFurniture(nt1Data, nt2Data, newFurniture) {
  if (nt1Data && nt1Data.length > 0) {
    setJson('aams_residential_NT1', nt1Data)
  }
  if (nt2Data && nt2Data.length > 0) {
    setJson('aams_residential_NT2', nt2Data)
  }
  if (newFurniture && newFurniture.length > 0) {
    const existingLib = getFurnitureLibrary()
  ...
  addAuditLogEntry(
    'Excel Data Imported',
    `Imported ${nt1Data.length} NT1 units (${gh1} guesthouses) and ${nt2Data.length} NT2 units (${gh2} guesthouses) from Excel spreadsheet.`,
    'settings'
  )
}

// API to get all guest history (bookings) for a given residential room
// guesthouses that appear in residential are also tracked via guest bookings
export function getResidentialRoomGuestHistory(buildingCode, roomNo) {
  const allBookings = getJson('aams_guest_bookings', DEFAULT_GUEST_BOOKINGS)
  // Match bookings by roomNo only (guesthouse rooms may share room numbers across sections)
  return allBookings.filter((b) => b.roomNo === roomNo)
}

// Sync guesthouse residential units into guest_rooms store
// Call this after importing so the NT1 guesthouses are visible in guest-houses tab
export function syncGuesthouseResidentialToGuestRooms(buildingCode) {
  const code = buildingCode.toUpperCase()
  const residential = getResidentialUnits(code)
  const ghResidential = residential.filter((u) => u.isGuesthouse === true)

  if (ghResidential.length === 0) return

  const allRooms = getJson('aams_guest_rooms', DEFAULT_GUEST_ROOMS)
  const existingRooms = allRooms[code] || []

  ghResidential.forEach((unit) => {
    const exists = existingRooms.some((r) => r.roomNo === unit.roomNo)
    if (!exists) {
      existingRooms.push({
        floor: `Floor ${unit.floor}`,
        roomNo: unit.roomNo,
        furniture: unit.furniture && unit.furniture !== 'NIL'
          ? unit.furniture.split(',').map((f) => f.trim())
          : [],
        status: unit.occupancy === 'Occupied' ? 'Occupied' : 'Available',
        maintenance: 'none',
        checkInStatus: unit.occupancy === 'Occupied' ? 'Completed' : 'Pending',
        checkOutStatus: 'Pending',
        isResidentialGuesthouse: true,
      })
    }
  })

  allRooms[code] = existingRooms
  setJson('aams_guest_rooms', allRooms)
}
