...
export default function Reports() {
  ...
  const uniqueDepartments = useMemo(() => {
    const depts = new Set()
    residentialNT1.forEach((u) => u.deptt && depts.add(u.deptt.trim()))
    residentialNT2.forEach((u) => u.deptt && depts.add(u.deptt.trim()))
    bookings.forEach((b) => b.guestStatus && depts.add(b.guestStatus.trim()))
    return Array.from(depts).filter((d) => d && d !== '-').sort()
  }, [residentialNT1, residentialNT2, bookings])
  ...
  const customReportData = useMemo(() => {
    ...
    const { target, building, status, furnitureFilter, selectedDept = 'All' } = customReportConfig
    ...
    if (target === 'residential') {
      ...
      const getActive = (roomNo) => bookings.find(b => b.roomNo === roomNo && (b.bookingStatus === 'Occupied' || b.bookingStatus === 'Reserved'))
      if (building === 'All' || building === 'NT1') {
        pool = [...pool, ...guestHouseNT1.map(u => {
          const active = getActive(u.roomNo)
          return {
            ...u,
            type: 'Guest House',
            name: 'NT1',
            residentName: active ? active.guestName : '-',
            deptt: active ? active.guestStatus : '-',
          }
        })]
      }
      if (building === 'All' || building === 'NT2') {
        pool = [...pool, ...guestHouseNT2.map(u => {
          const active = getActive(u.roomNo)
          return {
            ...u,
            type: 'Guest House',
    ...
    if (selectedDept !== 'All') {
      pool = pool.filter((u) => {
        return u.deptt && u.deptt.toLowerCase() === selectedDept.toLowerCase()
      })
    }
    ...
    return pool
  }, [selected, customReportConfig, residentialNT1, residentialNT2, guestHouseNT1, guestHouseNT2, bookings])
  ...
  const handleExportReport = () => {
    if (selected === 'occupancy') {
      ...
      const rows = reportRows.map((r) => {
        ...
        if (cols.includes('deptt')) item.deptt = r.deptt || '-'
                    ...
                    <tr className="border-b border-outline-variant/65">
                      ...
                      {selected === 'custom' && customReportConfig && (
                        <>
                          ...
                          {customReportConfig.columns.includes('deptt') && <th className="px-gutter py-4 text-xs font-bold uppercase text-secondary">DEPARTMENT</th>}
                      ...
                      <tr key={idx} className="transition-colors hover:bg-surface-container-low/30">
                        ...
                        {selected === 'custom' && customReportConfig && (
                          <>
                            ...
                            {customReportConfig.columns.includes('deptt') && <td className="px-gutter py-4 text-sm text-on-surface-variant font-medium">{row.deptt || '-'}</td>}
